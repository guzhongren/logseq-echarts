...
export async function findCodeBlock(
  rendererBlockIdentity: BlockIdentity,
): Promise<[BlockUUID, CodeBlockContent] | null> {
  let block = await logseq.Editor.getBlock(rendererBlockIdentity, {
...
export function isRendererBlock(
  blockEntity: BlockEntity | null | undefined,
): boolean {
  if (!blockEntity) {
    return false
  }
